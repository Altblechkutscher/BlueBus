    ...
    if (messageLength > 0) {
        ...
        if (strcmp(msgBuf[0], "ABS_VOL") == 0) {
            ...
            if (strcmp(msgBuf[2], "TITLE:") == 0) {
                ...
                strncpy(bt->title, title, BT_METADATA_FIELD_SIZE);
                ...
                strncpy(bt->artist, artist, BT_METADATA_FIELD_SIZE);
            } else {
                if (strcmp(msgBuf[2], "ALBUM:") == 0) {
                    ...
                    strncpy(bt->album, album, BT_METADATA_FIELD_SIZE);
                }
                if (bt->metadataStatus == BT_METADATA_STATUS_NEW) {
                    LogDebug(
                        LOG_SOURCE_BT, 
            ...
            if (bt->activeDevice.deviceId == 0) {
                ...
                if (deviceName != 0) {
                    strncpy(bt->activeDevice.deviceName, deviceName, BT_DEVICE_NAME_LEN - 1);
                } else {
            ...
            if (nameLen > BC127_DEVICE_NAME_OFFSET) {
                ...
                if (memcmp(macId, bt->activeDevice.macId, BT_MAC_ID_LEN) == 0 &&
                    ...
                    strncpy(bt->activeDevice.deviceName, name, BT_DEVICE_NAME_LEN - 1);
                    bt->status = BT_STATUS_CONNECTED;
                    EventTriggerCallback(BT_EVENT_DEVICE_CONNECTED, 0);
                }
