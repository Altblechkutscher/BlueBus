...
{
    LogWarning("HandlerBTDeviceFound()");
    HandlerContext_t *context = (HandlerContext_t *) ctx;
        ...
        if (context->bt->type == BT_BTM_TYPE_BC127) {
            ...
            if (context->bt->pairedDevicesCount > 0) {
                if (context->btSelectedDevice == HANDLER_BT_SELECTED_DEVICE_NONE) {
                    BM83CommandLinkBackLastDevice(context->bt);
...
void HandlerBTDeviceDisconnected(void *ctx, uint8_t *data)
{
    ...
    //BTClearPairingErrors(context->bt);
    if (context->ibus->ignitionStatus > IBUS_IGNITION_OFF) {
        if (context->btSelectedDevice != HANDLER_BT_SELECTED_DEVICE_NONE) {
...
void HandlerBTBM83AVRCPUpdates(void *ctx, uint8_t *data)
{
    ...
    if (type == BM83_AVRCP_EVT_PLAYBACK_STATUS_CHANGED &&
        ...
        if (status == 0x01) {
            ...
            context->bt->metadataStatus = BT_METADATA_STATUS_GET;
...
void HandlerBTBM83LinkBackStatus(void *ctx, uint8_t *pkt)
{
    HandlerContext_t *context = (HandlerContext_t *) ctx;
    if (context->ibus->ignitionStatus != IBUS_IGNITION_OFF) {
        if (pkt[BM83_FRAME_DB0] == BM83_DATA_LINK_BACK_ACL &&
            pkt[BM83_FRAME_DB1] == BM83_DATA_LINK_BACK_ACL_FAILED
        ) {
            if (context->btSelectedDevice == HANDLER_BT_SELECTED_DEVICE_NONE) {
                context->btSelectedDevice = 0;
            }
 ...
 * HandlerBTBM83BootStatus()
 *     Description:
 *         When the BM83 reports that it is in standby mode, we can
 *         reasonably deduce that the module is disconnected from any
 *         device. At this point, begin trying to connect to any paired devices.
 *         If there are no paired devices listed, then query the device for the
 *         PDL
 *     Params:
 *         void *ctx - The context provided at registration
 *         uint8_t *tmp - Any event data
 ...
 *         void
 */
void HandlerBTBM83BootStatus(void *ctx, uint8_t *tmp)
{
    HandlerContext_t *context = (HandlerContext_t *) ctx;
    if (context->bt->pairedDevicesCount == 0 &&
    ...
    if (context->bt->powerState == BT_STATE_STANDBY &&
        context->ibus->ignitionStatus > IBUS_IGNITION_OFF &&
        context->bt->discoverable == BT_STATE_OFF &&
        context->bt->pairedDevicesCount > 0
    ) {
        if (context->btSelectedDevice == HANDLER_BT_SELECTED_DEVICE_NONE ||
            context->btSelectedDevice > context->bt->pairedDevicesCount - 1
        ) {
            context->btSelectedDevice = 0;
        }
        BTPairedDevice_t *dev = &context->bt->pairedDevices[context->btSelectedDevice];
 ...
 *         void
 */
void HandlerTimerBTBM83AVRCPRegisterStatusNotifier(void *ctx)
{
    HandlerContext_t *context = (HandlerContext_t *) ctx;
    if (context->bt->metadataStatus == BT_METADATA_STATUS_GET) {
        context->bt->metadataStatus = BT_METADATA_STATUS_SET;
        BM83CommandAVRCPRegisterNotification(
            context->bt,
            BM83_AVRCP_EVT_PLAYBACK_TRACK_CHANGED
        );
...
void HandlerTimerBTBM83ManagePowerState(void *ctx)
{
    HandlerContext_t *context = (HandlerContext_t *) ctx;
    if (context->bt->powerState == BT_STATE_OFF) {
        if (context->btBootState == HANDLER_BT_BOOT_MFB_L) {
            BT_DATA_SEL = 1;
            ...
            BM83CommandReadLinkStatus(context->bt);
            BT_DATA_SEL = 0;
